...
export const getStaticProps: GetStaticProps = async () => {
    const { data, error } = await supabase.from('posts').select().filter('type', 'eq', 'reviews')
    return {
        props: { data, error },
        revalidate: 5
    }
...
const Reviews: React.FC<feedProps> = (props) => {
                ...
                <main>
                    {props.data.map(review => (
                        <div key={review.id} className="mt-4">
                            <div className='p-8 cursor-pointer border-2 border-b-8 border-green-300' onClick={() => router.push(`/reviews/${review.id}`)}>
                                <h2>{review.title}</h2>
                                <small>By {review.user_email ? review.user_email : "Unknown author"} on {review.inserted_at}</small>
