...
export const getStaticProps: GetStaticProps = async () => {
    const { data, error } = await supabase.from('posts').select().filter('type', 'eq', 'recaps')
    return {
        props: { data, error },
        revalidate: 5
    }
...
const Recaps: React.FC<feedProps> = (props) => {
    ...
    return (
        <Layout>
            ...
            <div>
                <h1>Recaps</h1>
                <main>
                    {props.data.map(recap => (
                        <div key={recap.id} className="mt-4">
                            <div className='p-8 cursor-pointer border-2 border-b-8 border-green-300' onClick={() => router.push(`/recpas/${recap.id}`)}>
                                <h2>{recap.title}</h2>
                                <small>By {recap.user_email ? recap.user_email : "Unknown author"} on {recap.inserted_at}</small>
